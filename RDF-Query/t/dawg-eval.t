...
sub eval_test {
	...
	TODO: {
		...
		my $ok	= eval {
			...
			my $type		= (blessed($actual) and $actual->isa( 'RDF::SPARQLResults::Graph' )) ? 'graph' : '';
...
sub compare_results {
	...
	if (not(ref($actual))) {
		...
		return $ok;
	} elsif (blessed($actual) and $actual->isa('RDF::SPARQLResults::Graph')) {
		die unless (blessed($expected) and $expected->isa('RDF::SPARQLResults::Graph'));
