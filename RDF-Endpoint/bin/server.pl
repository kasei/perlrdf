...
my $s	= RDF::Endpoint::Server->new( $model,
			Port		=> $port,
			Prefix		=> '',
			CGI			=> $cgi,
...
if ($announce) {
	my $threads		= eval "require threads";
	unless ($threads) {
		warn "To announce the SPARQL endpoint with Rendezvous, you must be using a Perl built with support for threads.\n\n";
		exit(1);
	}
	
	my $rendezvous	= eval "require Net::Rendezvous::Publish";
	unless ($rendezvous) {
		warn "To announce the SPARQL endpoint with Rendezvous, the Net::Rendezvous::Publish module must be installed.\n\n";
		exit(1);
	}
	
	require Net::Rendezvous::Publish;
	threads->create(sub{
		my $publisher = Net::Rendezvous::Publish->new or die "couldn't make a Responder object";
