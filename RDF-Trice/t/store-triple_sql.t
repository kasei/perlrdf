...
my $s		= RDF::Trice::Node::Resource->new( 'http://example.com/' );
my $title	= RDF::Trice::Node::Resource->new( $dc->title );
my $desc	= RDF::Trice::Node::Resource->new( $dc->description );
my $v1		= RDF::Trice::Node::Variable->new( 'title' );
my $v2		= RDF::Trice::Node::Variable->new( 'description' );

{
	my $triple	= RDF::Trice::Statement->new($s, $title, $v1);
	my $store	= RDF::Trice::Store::DBI->new('temp');
...
SKIP: {
	eval "use RDF::Query::Algebra";
	skip "RDF::Query not available", 1 if ($@);
...

{
	my $triple	= RDF::Trice::Statement->new($s, $title, $v1);
	my $store	= RDF::Trice::Store::DBI->new('temp');
	my $ctx		= RDF::Trice::Node::Resource->new( 'http://example.com/' );
