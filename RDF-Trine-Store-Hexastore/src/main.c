	...
	if (1) {
		int writecount	= 0;
		{
			...
			for (int j = 10; j > 5; j--) {
				...
				for (int i = j; i > 0; i--) {
					writecount++;
					hx_terminal_add_node( t, (rdf_node_id) i );
				}
				hx_vector_add_terminal( v, (rdf_node_id) j, t );
			}
...
void nodemap_test ( void ) {
	...
	for (char c = 'z'; c >= 'a'; c--) {
		sprintf( s, "%c", c );
		rdf_node_id n	= hx_nodemap_get_node_id( m, s );
		fprintf( stderr, "%c -> %d\n", c, (int) n );
	}
	
	char* nodestr	= hx_nodemap_get_node_string( m, (rdf_node_id) 7 );
...
void hexastore_test ( void ) {
		...
		
		{
			...
			for (int i = 1; i <= 400; i++) {
				for (int j = 1; j <= 100; j++) {
					for (int k = 1; k <= 50; k++) {
						count++;
						hx_add_triple( hx, (rdf_node_id) i, (rdf_node_id) j, (rdf_node_id) k );
						if (count % 25000 == 0)
		...
		fprintf( stderr, "removing triples...\n" );
		{
			...
			for (int i = 1; i <= 9; i++) {
				for (int j = 1; j <= 9; j++) {
					for (int k = 1; k <= 9; k++) {
						count++;
						hx_remove_triple( hx, (rdf_node_id) i, (rdf_node_id) j, (rdf_node_id) k );
					}
				}
			}
	...
	if (0) {
...

		{
			...
			int count	= 1;
			hx_index_iter* iter	= hx_get_statements( hx, (rdf_node_id) 4, (rdf_node_id) 0, (rdf_node_id) 0, HX_OBJECT );
			while (!hx_index_iter_finished( iter )) {
				rdf_node_id s, p, o;
				hx_index_iter_current( iter, &s, &p, &o );
				fprintf( stderr, "[%d] %d, %d, %d\n", count++, (int) s, (int) p, (int) o );
		...
		}
		{
			...
			int count	= 1;
			hx_index_iter* iter	= hx_get_statements( hx, (rdf_node_id) 0, (rdf_node_id) 0, (rdf_node_id) 9, HX_PREDICATE );
			while (!hx_index_iter_finished( iter )) {
				rdf_node_id s, p, o;
				hx_index_iter_current( iter, &s, &p, &o );
				fprintf( stderr, "[%d] %d, %d, %d\n", count++, (int) s, (int) p, (int) o );
		...
		}
		{
			...
			int count	= 1;
			hx_index_iter* iter	= hx_get_statements( hx, (rdf_node_id) 0, (rdf_node_id) 0, (rdf_node_id) 0, HX_PREDICATE );
			while (!hx_index_iter_finished( iter )) {
				rdf_node_id s, p, o;
				hx_index_iter_current( iter, &s, &p, &o );
				fprintf( stderr, "[%d] %d, %d, %d\n", count++, (int) s, (int) p, (int) o );
...
void memory_test (void) {
	...
	for (int i = 100; i > 0; i--) {
		hx_vector* v	= hx_new_vector();
		hx_head_add_vector( h, (rdf_node_id) i, v );
		for (int j = 200; j > 0; j--) {
			hx_terminal* t	= hx_new_terminal();
			hx_vector_add_terminal( v, (rdf_node_id) j, t );
			for (int k = 1; k < 25; k++) {
//				fprintf( stderr, "%d %d %d\n", (int) i, (int) j, (int) k );
				hx_terminal_add_node( t, (rdf_node_id) k );
			}
		}
	}
...
void index_test (void) {
	...
	hx_index_add_triple( index, (rdf_node_id) 1, (rdf_node_id) 2, (rdf_node_id) 3 );
	hx_index_debug( index );
	
	for (int i = 1; i < 4; i++) {
		for (int j = 4; j <= 6; j++) {
			for (int k = 7; k <= 8; k++) {
				hx_index_add_triple( index, (rdf_node_id) i, (rdf_node_id) j, (rdf_node_id) k );
			}
		}
	}
	hx_index_debug( index );
	...
	hx_index_remove_triple( index, (rdf_node_id) 0, (rdf_node_id) 4, (rdf_node_id) 7 );
	hx_index_remove_triple( index, (rdf_node_id) 0, (rdf_node_id) 4, (rdf_node_id) 8 );
	fprintf( stderr, "total triples: %d\n", (int) hx_index_triples_count( index ) );
...
void head_test (void) {
	...
	hx_vector* v	= hx_new_vector();
	hx_head_add_vector( h, (rdf_node_id) 1, v );
	hx_head_debug( "", h );
	
	{
		hx_terminal* l	= hx_new_terminal();
		hx_vector_add_terminal( v, (rdf_node_id) 3, l );
		hx_head_debug( "", h );
		for (int i = 1; i < 8; i++) {
			hx_terminal_add_node( l, (rdf_node_id) i );
		}
		hx_head_debug( "", h );
	}
	...
	{
		hx_terminal* l	= hx_new_terminal();
		hx_vector_add_terminal( v, (rdf_node_id) 1, l );
		hx_head_debug( "", h );
		for (int i = 5; i < 9; i++) {
			hx_terminal_add_node( l, (rdf_node_id) i );
		}
	...
	for (int i = 499; i > 0; i--) {
		hx_head_remove_vector( h, (rdf_node_id) i );
	}
	fprintf( stderr, "size: %d\n", (int) hx_head_size( h ) );
	...
	for (int k = 1; k < 3; k++) {
		...
		for (int i = 1; i <= 10; i++) {
			...
			for (int j = 1; j < (1 + rand() % 10); j++) {
				hx_terminal_add_node( l, (rdf_node_id) j );
			}
			hx_vector_add_terminal( v, (rdf_node_id) i, l );
		}
		hx_head_add_vector( h, (rdf_node_id) k, v );
	}
...
void vector_test (void) {
	...
	hx_vector_add_terminal( v, (rdf_node_id) 3, l );
	hx_vector_debug( "- ", v );
	hx_terminal_add_node( l, (rdf_node_id) 7 );
	hx_vector_debug( "- ", v );
	hx_vector_add_terminal( v, (rdf_node_id) 2, l );
	hx_vector_debug( "- ", v );
	hx_terminal_add_node( l, (rdf_node_id) 8 );
	hx_vector_debug( "- ", v );
	hx_terminal_add_node( l, (rdf_node_id) 9 );
	hx_vector_debug( "- ", v );
	
	fprintf( stderr, "size: %d\n", (int) hx_vector_size( v ) );
	hx_vector_remove_terminal( v, (rdf_node_id) 3 );
	fprintf( stderr, "size: %d\n", (int) hx_vector_size( v ) );
	hx_vector_debug( "- ", v );
	
	for (int i = 1; i < 400; i++) {
		hx_terminal* l	= hx_new_terminal();
		if (hx_vector_add_terminal( v, (rdf_node_id) i, l ) != 0) {
			hx_free_terminal( l );
		}
	}
	...
	for (int i = 399; i >= 0; i--) {
		hx_vector_remove_terminal( v, (rdf_node_id) i );
	}
	fprintf( stderr, "size: %d\n", (int) hx_vector_size( v ) );
...
void terminal_test (void) {
	...
	hx_terminal_add_node( l, (rdf_node_id) 5 );
	hx_terminal_debug( "- ", l, 1 );

	hx_terminal_add_node( l, (rdf_node_id) 1 );
	hx_terminal_debug( "- ", l, 1 );

	hx_terminal_add_node( l, (rdf_node_id) 2 );
	hx_terminal_debug( "- ", l, 1 );
	...
	hx_terminal_add_node( l, (rdf_node_id) 3 );
	hx_terminal_debug( "- ", l, 1 );

	r	= hx_terminal_binary_search( l, n, &i );
	printf( "search: %d %d\n", r, i );
	
	hx_terminal_remove_node( l, (rdf_node_id) 2 );
	hx_terminal_debug( "- ", l, 1 );
	hx_terminal_remove_node( l, (rdf_node_id) 3 );
	hx_terminal_debug( "- ", l, 1 );
	hx_terminal_remove_node( l, (rdf_node_id) 5 );
	hx_terminal_debug( "- ", l, 1 );
	hx_terminal_remove_node( l, (rdf_node_id) 6 );
	hx_terminal_debug( "- ", l, 1 );
	hx_terminal_remove_node( l, (rdf_node_id) 1 );
	hx_terminal_debug( "- ", l, 1 );
...
// 		hx_terminal_debug( "- ", l, 1 );
	...
	for (int i = 101; i < 200; i++) {
		hx_terminal_remove_node( l, (rdf_node_id) i );
// 		hx_terminal_debug( "- ", l, 1 );
	}
	for (int i = 100; i >= 0; i--) {
		hx_terminal_remove_node( l, (rdf_node_id) i );
