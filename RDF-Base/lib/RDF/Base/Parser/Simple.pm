...
RDF::Base::Parser::Simple - Base class for RDF parser classes.


=head1 VERSION

This document describes RDF::Base::Parser::Simple version 0.0.1


=head1 SYNOPSIS

    use RDF::Base::Parser::Simple;
...
package RDF::Base::Parser::Simple;
...
Returns a new simple rdf parser.
...
sub parse_as_stream ($$) {
	...
	return $self->parse_string_as_stream( $content, $base );
}
...
sub parse_into_model ($$$;$) {
	...
	my $base	= shift;
	my $model	= shift;
	my $stream	= $self->parse_as_stream( $source, $base );
...
sub parse_string_as_stream ($$) {
	...
	my $stream	= RDF::Base::Iterator::Statement->new( sub {
		...
		my $line	= substr($content, 0, index($content, "\n") + 1, '');
		chomp( $line );
		my $st		= RDF::Base::Statement->parse( $line );
		return $st;
	} );
	return $stream;
}
...
sub parse_string_into_model ($$$;$) {
	...
	my $base	= shift;
	my $model	= shift;
	my $stream	= $self->parse_string_as_stream( $content, $base );
	while (my $st = $stream->next) {
		$model->add_statement( $st );
	}
}
...
RDF::Base::Parser::Simple requires no configuration files or environment variables.
