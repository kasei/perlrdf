...
package RDF::Base::Parser::RDFXML;
...
sub parse_as_stream ($$) {
	...
	my $iter		= smap { $_->{context} = $base_uri; $_ } coerce( 'RDF::Base::Iterator::Statement', $stream );
...
sub parse_string_into_model ($$$;$) {
	...
	my $stream	= $self->parse_string_as_stream( $string, $base_uri );
	while (my $st = $stream->next) {
		$model->add_statement( $st );
	}
}
